...
function captureHandDefaultFromCurrent3D(side) {
  ...
  const wrist = jointWorldPosSpineLocal3D(side, 'Wrist');
...
function buildBody3D() {
  ...
  function buildArmSide(group, pivot, side) {
    ...
    // captureHandDefaultFromCurrent3D) and segment lengths, so the position-based arm
...
function applyPose3D(poseName, { reframe = false } = {}) {
  ...
  // see captureHandDefaultFromCurrent3D/applyArmPosition3D) fully determine their own
